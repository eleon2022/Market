...
async def sell_phone(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lang = context.user_data.get("lang", "العربية")
    msg = "أرسل صورة المنتج أو اضغط /skip للتخطي:" if lang == "العربية" else "وێنە بنێرە یان بنووسە /skip:"
    await update.message.reply_text(msg)
    return SELL_PHOTO

async def receive_image(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # حفظ الصورة باستخدام file_id مباشرة
...
async def invalid_photo_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lang = context.user_data.get("lang", "العربية")
    msg = "يرجى إرسال صورة أو الضغط على /skip." if lang == "العربية" else "تکایە وێنە بنێرە یان /skip بنووسە."
    await update.message.reply_text(msg)
    return SELL_PHOTO
